...
exports.getResourcesLD = async (data) => {
    try {
        var pageNo = 0;
        var startPos = 0;
        var pagesCount = 0;
        ...
        startPos = (pageNo * pageSize) - pageSize;
        //get records to display
        var data = await ResourcesDAO.getResourcesLD(data.search, data.categoryId, startPos + 1, pageSize + startPos);
        for (var i = 0; i < data.length; i++) {
            data[i].createdDate = convertFromObjToTimestamp(data[i].createdDate);
            data[i].updatedDate = convertFromObjToTimestamp(data[i].updatedDate);
        }
...
exports.getResourcesCategory = async () => {
    return await ResourcesDAO.getResourcesCategory();
}
