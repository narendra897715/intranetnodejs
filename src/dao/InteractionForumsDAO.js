...
exports.getCategoryDropDown = () => {
    return new Promise((resolve, reject) => {
        new sql.Request(poolPromise).execute('IntranetPortal_sp_getInteractionForumsCategoryList', (err, result) => {
            if (err) {
                logger.fatal(err);
                reject(err);
            }
...
exports.getSubMenu = () => {
    return new Promise((resolve, reject) => {
        new sql.Request(poolPromise).execute('IntranetPortal_sp_getInteractionForumsSubMenu', (err, result) => {
            if (err) {
                logger.fatal(err);
                reject(err);
            }
...
exports.getCategoryDropDownSelection = () => {
    return new Promise((resolve, reject) => {
        new sql.Request(poolPromise).execute('IntranetPortal_sp_getInteractionForumsCategoryListSelection', (err, result) => {
            if (err) {
                logger.fatal(err);
                reject(err);
            }
...
exports.getInteractionForumsNewlyAdded = (searchData, categoryId, filterId, rowStart, rowEnd) => {
    return new Promise((resolve, reject) => {
        new sql.Request(poolPromise)
            .input('searchData', sql.NVarChar, searchData)
            .input('categoryId', sql.Numeric, categoryId)
        ...
        return new Promise((resolve, reject) => {
            ...
            transaction.begin(err => {
                ...
                const request = new sql.Request(transaction)
                    ...
                    .query('update tbl_polling_options set optionDescription=@optionDescription,updatedBy=@updatedBy,updatedDate=@updatedDate where optionId=@optionId', (err, result) => {
                        if (err) {
                            reject(err);
                        }
...
exports.savePollingSuggestions = (data) => {
    return new Promise((resolve, reject) => {
        try {
            ...
            transaction.begin(err => {
                ...
                const request = new sql.Request(transaction)
                    .input('interactionForumsId', sql.Int, data.interactionForumsId)
                    .input('suggestedBy', sql.Int, data.suggestedBy)
                    .input('suggestedOption', sql.NVarChar, data.suggestedOption)
                    .input('suggestedDate', sql.DateTime, data.suggestedDate)
                    .query('insert into tbl_polling_suggestions (interactionForumsId,suggestedBy,suggestedOption,suggestedDate)' +
                        ' values (@interactionForumsId,@suggestedBy,@suggestedOption,@suggestedDate);SELECT SCOPE_IDENTITY() AS id;', (err, result) => {
                            if (err) {
                                reject(err);
                                // console.log(err);
                            }
...
exports.pollingSuggestions = (interactionForumsId) => {
    return new Promise((resolve, reject) => {
        new sql.Request(poolPromise).input('interactionForumsId', sql.Int, interactionForumsId)
            .execute('[dbo].[IntranetPortal_sp_pollingsuggestedOption ]', (err, result) => {
                if (err) {
                    logger.fatal(err);
                    reject(err);
                }
