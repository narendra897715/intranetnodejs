...
exports.sendAppreciation = (data) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        transaction.begin(err => {
            ...
            ps.prepare('insert into tbl_employees_appreciation_wall (appreciationTo,appreciatedBy,contentOfAppreciation,createdBy,createdDate,updatedDate,projectId) values (@appreciationTo,@appreciatedBy,@contentOfAppreciation,@createdBy,@createdDate,@updatedDate,@projectId) ', err => {
...
exports.projectsList = (data) => {
    return new Promise((resolve, reject) => {
        new sql.Request(poolPromise)
        .input('searchData', sql.NVarChar, data.searchData)
            .execute('[dbo].[IntranetPortal_sp_getProjectsList]', (err, result) => {
                if (err) {
                    logger.fatal(err);
                    reject(err);
                }
